...
pub async fn send_duel_err(challenger: &str, client: &mut tmi::Client, msg: tmi::Privmsg<'_>, err: &str) -> anyhow::Result<()>{
  send_msg(client, &msg, &format!("@{} Error; {}", challenger, err )).await
}

pub async fn send_msg(client: &mut tmi::Client, msg: &tmi::Privmsg<'_>, text: &str) -> anyhow::Result<(), anyhow::Error> {
    client
...
pub async fn on_msg(client: &mut tmi::Client, msg: tmi::Privmsg<'_>, bot_state: &mut State) -> anyhow::Result<()> {
...
pub async fn reply_to(client: &mut tmi::Client, msg: &tmi::Privmsg<'_>, reply:&str) -> anyhow::Result<(), anyhow::Error> {
    client
